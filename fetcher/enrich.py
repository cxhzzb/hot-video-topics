...
- power 字段评估该热点的"电力宣传植入适配度"：能否自然关联电力元素（迎峰度夏/度冬保供电、台风洪涝电力抢修、电网工程与特高压、新能源与充电桩、安全用电科普、用电量反映经济活力、大型活动保电等）。10 分=天然电力话题（如高温用电创新高、台风致停电抢修），0 分=完全无法关联。涉及人员伤亡、灾难、重大负面舆情的话题必须保守打分（一般不超过3分），避免"灾难营销"引发反感
...
[
  {{
    ...
    "power": {{"score": 电力宣传植入适配度0-10的整数, "angle": "分数≥4时给出具体植入角度（一句话，如'高温持续→电网负荷创新高→致敬一线保电工人'）；分数<4填'不建议植入'"}}
  }}
]"""
...
def enrich_topics(topics, cache):
    ...
    for topic in topics[:ENRICH_TOP_N]:
        ...
        if cached and "power" in cached:  # 旧缓存缺 power 字段，需重新加工一次
            topic["ai"] = cached
            reused += 1
        else:
            todo.append(topic)
